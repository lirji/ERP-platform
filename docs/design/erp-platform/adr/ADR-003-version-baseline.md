...
主线已至 4.1.x，按 Spring 的常规支持节奏，3.3 线的 OSS 支持窗口应已关闭，3.3.11 很可能是该线最后一个公开补丁。这意味着**后续安全修复不会再进入 3.3.x**。

本 ADR **不**在此刻做升级决策，因为：
- 升级到 3.5.x / 4.x 会脱离 `workflow-platform` 的 minor 线，P8 集成成本需要重新评估；
- MyBatis-Plus 对 Spring Boot 4 的稳定支持状况尚未核验（当前 starter 明确命名为 `spring-boot3`）；
- P0 骨架尚无业务代码，**升级成本在 P1–P3 期间仍然很低**，现在锁定反而放弃了信息。

**升级决策点**：P1 结束前必须复核并给出结论。触发条件：出现影响本系统的 3.3.x 未修复 CVE 时立即升级，不等 P1。
