...
import com.lrj.erp.sales.domain.*;
...
public class SalesReturnService {
    public static final String DOC_TYPE="SALES_RETURN";
    private static final String SALES_SHIPMENT_TYPE="SALES_SHIPMENT";
    ...
    public SalesReturnService(SalesReturnRepository repository,StockPostingService posting,ApprovalPort approvals,
    ...
    public long create(long tenantId,long sourceLineId,BigDecimal quantity,String batchNo,String commandId,String reason,long operator) {
        ...
        // 销售回库批次必须来自调用方明确选择，不自动沿用原批次。
        return repository.insert(new Return(0,tenantId,numbers.next(tenantId,"SR",LocalDate.now()),sourceLineId,
    ...
    public void execute(long tenantId,long id,long operator) {
        ...
        // 回库恢复原出库成本，显式退货批次可以不同；不能用售价或当前均价替代历史成本。
        BigDecimal original=posting.sourceValue(tenantId,SALES_SHIPMENT_TYPE,""+s.sourceId(),""+s.lineId());
        if(original!=null) incomingValue=portion(original.negate(),s.returnedQty().add(r.quantity()),s.quantity(),6)
                .subtract(portion(original.negate(),s.returnedQty(),s.quantity(),6));
        InventoryBucket bucket=InventoryBucket.ofBatch(tenantId,s.companyId(),s.warehouseId(),s.skuId(),r.batchNo());
        posting.post(new PostingRequest(bucket,PostingDirection.IN,r.quantity(),DOC_TYPE,DOC_TYPE,""+id,"1",operator,incomingValue));
        repository.amount(r,amount);
        move(required(tenantId,id),DocumentEvent.FINISH,operator);
        outbox.record(tenantId,DOC_TYPE,""+id,ReturnPosted.SALES,
                new ReturnPosted(SALES_SHIPMENT_TYPE,""+s.sourceId(),DOC_TYPE,""+id,r.documentNo(),s.companyId(),s.partnerId(),amount,s.currency(),operator));
        outbox.record(tenantId,DOC_TYPE,""+id,"SalesReturnLinked.v1",Map.of(
                "parentType",SALES_SHIPMENT_TYPE,"parentId",""+s.sourceId(),"parentNo",s.sourceNo(),
    ...
    private void require(boolean condition,String reason) {
        if(!condition) throw new DomainException(SalesErrorCode.INVALID_RETURN,Map.of("reason",reason));
    }
}
