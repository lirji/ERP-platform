...
| 项 | 说明 |
|---|---|
| `RISK-VER-01` | Spring Boot 3.3.x 疑似已退出 OSS 维护。当前用 3.3.11（该线最新补丁）。**P1 结束前必须复核升级** |
| `origin` remote | `git@github.com:lirji/ERP-platform.git` 存在于 `.git/config` 但**不可达**，且非本会话配置。push 未执行，待用户确认 |
