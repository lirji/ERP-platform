...
|---|---|---|---|---|---|
| `RISK-VER-01` | **Spring Boot 3.3.x 很可能已退出 OSS 维护**。主线已至 4.1.x，3.3.11 很可能是该线最后一个公开补丁，后续安全修复不再进入 3.3.x | 高 | 中（当前无业务代码，升级成本低；上线后升高） | 采用 3.3.11（该线最新补丁），**不在 P0 做升级决策**——P0 尚无业务代码，升级成本在 P1–P3 仍然很低，此刻锁定反而放弃信息。见 `ADR-003` | ① P1 结束前必须复核并给出升级结论；② 出现影响本系统的 3.3.x 未修复 CVE 时**立即升级，不等 P1** |
