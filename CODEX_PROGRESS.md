...
继续原计划 P7–P11；P8 仅在真实复杂审批需求触发时实施。用户已确认：退货生成独立红字调整及待退款记录，保留原收付款事实。当前分支 `feat/p7-inventory-reverse`，先实施 P7 库存计价，再退货/调拨/盘点。

历史目标：接手 Claude 会话 `6ce8fae5-f198-48db-ae23-48bd5dbc80c8`，用户已明确完成 P5 后继续 P6，做到采购到付款、订单到收款两条 MVP 业务闭环。

## 已完成

- P7.2 调拨/冻结盘点/审批调整完成，133 项全量回归通过（证据 TEST_RESULT-P7-2.json）；尚未完成 P7.3 退货。
...
请先读取 `CODEX_PROGRESS.md`。P5、P6 及 Git 交付已完成，不要重复实施。依据新的明确目标继续；从 P7 当前切片继续，再推进 P9–P11，P8 按条件执行。生产 OIDC/HTTP 与历史补账边界仍需保持明确，未执行生产部署。
